...
This guide covers migrating from OpenClaw (and other frameworks) to LibreFang. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.librefang/`.

### Options

```bash
# Specify a custom source directory
librefang migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.librefang/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
librefang migrate --from langchain   # Coming soon
librefang migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (LibreFang) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.librefang/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.librefang/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.librefang/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.librefang/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.librefang/config.toml`.
...
```bash
librefang skill install <skill-name-or-path>
```

LibreFang automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize LibreFang

```bash
librefang init
```

This creates `~/.librefang/` with a default `config.toml`.
...
**LibreFang** (add to `~/.librefang/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to LibreFang agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.librefang/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | LibreFang |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.librefang/config.toml` |
...
Tools were renamed between OpenClaw and LibreFang for consistency. The migration engine handles this automatically.

| OpenClaw Tool | LibreFang Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | LibreFang Tools |
|------------------|----------------|
...
| OpenClaw Name | LibreFang Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (LibreFang Protocol) peer-to-peer |
...
```bash
librefang migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
librefang skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.librefang/channels_import.toml
# Copy the [channels.*] sections into ~/.librefang/config.toml
```
