...
fn test_memory_store_recall_isolates_peer_namespaces() {
    ...
    kh.memory_store("key1", serde_json::json!("global_val"), None, None)
        .expect("store global");
    kh.memory_store(
        "key1",
        serde_json::json!("peer_a_val"),
        None,
        Some("peer-a"),
    )
    .expect("store peer-a");
    ...
    assert_eq!(
        kh.memory_recall("key1", None, None).expect("recall global"),
        Some(serde_json::json!("global_val"))
    );
    assert_eq!(
        kh.memory_recall("key1", None, Some("peer-a"))
            .expect("recall peer-a"),
        Some(serde_json::json!("peer_a_val"))
    );
    assert_eq!(
        kh.memory_recall("key1", None, Some("peer-b"))
            .expect("recall peer-b"),
        Some(serde_json::json!("peer_b_val"))
    );
    assert_eq!(
        kh.memory_recall("key1", None, Some("peer-c"))
...
fn test_memory_list_separates_global_and_peer_keys() {
    ...
    kh.memory_store("g1", serde_json::json!(1), None, None)
        .expect("store g1");
    kh.memory_store("g2", serde_json::json!(2), None, None)
        .expect("store g2");
    kh.memory_store("p1", serde_json::json!(3), None, Some("peer-a"))
        .expect("store p1");
    kh.memory_store("p2", serde_json::json!(4), None, Some("peer-a"))
        .expect("store p2");
    ...
    let peer_keys = kh.memory_list(None, Some("peer-a")).expect("list peer-a");
